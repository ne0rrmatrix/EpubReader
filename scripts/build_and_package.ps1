...
Generates Windows and Android builds, zips artifacts, and creates a markdown
report listing files and checksums.

Usage:
  pwsh ./scripts/build_and_package.ps1 [-RepoRoot <path>] [-KeystorePath
  <path>] [-AllowAndroidFailure]

Notes:
- Defaults assume this script lives in the repository under ./scripts and the
  solution is in the repo root.
...
if (-not $KeystorePath) {
    $KeystorePath = Join-Path -Path $RepoRoot `
        -ChildPath 'EpubReader\EpubReader.keystore'
}
...
# Ensure NuGet packages are restored for the solution before building
Write-Host "Restoring NuGet packages for solution: $solutionProj"
try {
    & dotnet restore $solutionProj
    if ($LASTEXITCODE -ne 0) { Write-Error "dotnet restore failed with exit code $LASTEXITCODE"; exit $LASTEXITCODE }
} catch { Write-Warning "dotnet restore encountered an error: $_" }

# MSBuild now provisions canonical google-services.json (if present in build-secrets).
# The packaging scripts no longer stage or clean that file; csproj handles it.
...
if ($LASTEXITCODE -ne 0) {
    Write-Error `
        "dotnet publish (windows) failed with exit code $LASTEXITCODE"
    exit $LASTEXITCODE
}
...
if ($LASTEXITCODE -ne 0) {
    if ($AllowAndroidFailure.IsPresent) {
        Write-Warning `
            "dotnet publish (android) failed with exit code " + `
            "$LASTEXITCODE, continuing because -AllowAndroidFailure " + `
            "was specified."
    } else {
        Write-Error `
            "dotnet publish (android) failed with exit code $LASTEXITCODE"
        exit $LASTEXITCODE
    }
}
...
Compress-Archive -Path (Join-Path $windowsOut '*') `
    -DestinationPath $windowsZip -Force
...
$apkFiles = Get-ChildItem -Path $androidOut -Recurse -File `
...
if ($apkFiles -and $apkFiles.Count -gt 0) {
    ...
    foreach ($f in $apkFiles) {
        Copy-Item -Path $f.FullName -Destination $tempApkDir
    }
    Compress-Archive -Path (Join-Path $tempApkDir '*') `
        -DestinationPath $androidZip -Force
    Remove-Item -Path $tempApkDir -Recurse -Force
    Write-Host "Created $androidZip with APK/AAB files"
} else {
    if (Test-Path $androidZip) { Remove-Item $androidZip -Force }
    Compress-Archive -Path (Join-Path $androidOut '*') `
        -DestinationPath $androidZip -Force
...
foreach ($z in @($windowsZip, $androidZip)) {
    if (Test-Path $z) {
        ...
        $reportLines += `
            "| $(Split-Path $z -Leaf) | $($info.Length) | $hash |"
    } else {
